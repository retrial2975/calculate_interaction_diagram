...
with st.sidebar:
    ...
    with st.expander("ขนาดหน้าตัด", expanded=True):
        b_in = st.number_input("ความกว้าง, b (cm)", value=40.0, min_value=1.0); h_in = st.number_input("ความลึก, h (cm)", value=60.0, min_value=1.0)
    with st.expander("ข้อมูลเหล็กเสริม", expanded=True):
        d_prime = st.number_input("d' (cm)", value=6.0, min_value=1.0); bar_dia_mm = st.selectbox("ขนาดเหล็กเสริม", [12, 16, 20, 25, 28, 32], index=3)
        nb = st.number_input("จำนวนเหล็กด้าน b (บน-ล่าง)", value=5, min_value=2); nh = st.number_input("จำนวนเหล็กด้าน h (ข้าง)", value=3, min_value=2)
    st.markdown("---")
...
# --- Main App Logic ---
steel_positions = generate_steel_positions(b_in, h_in, nb, nh, d_prime); bar_area = np.pi * (bar_dia_mm / 10.0 / 2)**2; Ast_total = len(steel_positions) * bar_area
if bending_axis == 'Y (Weak Axis)':
    calc_b, calc_h, axis_label, M_col = h_in, b_in, "Y (Weak)", "M2"; layers = get_layers_from_positions(steel_positions, 'Y')
else:
    calc_b, calc_h, axis_label, M_col = b_in, h_in, "X (Strong)", "M3"; layers = get_layers_from_positions(steel_positions, 'X')
    ...
    if df_loads is not None and 'selected_stories' in st.session_state and st.session_state.selected_stories and st.session_state.selected_columns:
        ...
        if not column_data.empty:
            ...
            if check_slenderness and st.session_state.story_lu_df is not None:
                lu_df = st.session_state.story_lu_df.rename(columns={'Lu (m)': 'Lu_m'}); column_data['Story'] = column_data['Story'].astype(str); lu_df['Story'] = lu_df['Story'].astype(str)
